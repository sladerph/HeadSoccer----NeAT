import {Connection, ConnectionHistory} from '@/Network/connection.js';
...
export default function Network(nb_inputs, nb_outputs, crossover) {
  this.nodes       = [];
  this.connections = [];
  this.network     = [];
  ...
  this.calculate = function(inputs) {
    ...
    for (let i = 0; i < this.nb_inputs; i++) {
    ...
    for (let i = 0; i < this.nb_outputs; i++) {
  ...
  this.addConnection = function(history) {
    if (this.isFullyConnected()) {
      window['console'].log('*** Unable to create a new connection !      ***');
      window['console'].log('*** The network is already fully connected ! ***');
      return;
    }

    // Choosing input node for the connection.
    do {
      var input = Math.floor(Math.random() * this.nodes.length);
      while (this.nodes[input].type == 'Output') {
        input = Math.floor(Math.random() * this.nodes.length);
      }

      var output = Math.floor(Math.random() * this.nodes.length);
      while (this.nodes[output].layer <= this.nodes[input].layer) {
        output = Math.floor(Math.random() * this.nodes.length);
      }
...
Network.prototype.addNode = function(history) {
  var rnd_con = Math.floor(Math.random() * this.connections.length);

  while (this.connections[rnd_con].input == getNode(this.nodes, this.bias) && this.connections.length != 1) {
    rnd_con = Math.floor(Math.random() * this.connections.length);
  }
...
Network.prototype.crossover = function(dad) {
  ...
  for (let i = 0; i < this.connections.length; i++) {
    ...
    if (index_matching_connection_dad != -1) { // Match found.
      if (!this.connections[i].enabled || !dad.connections[index_matching_connection_dad].enabled) {
        if (Math.random() < 0.75) { // Disable the child's connection 75% of the time.
  ...
  for (let i = 0; i < child_connections.length; i++) {
...
Network.prototype.mutate = function(history) {
  for (let i = 0; i < this.connections.length; i++) {
    if (Math.random() < change_weight_rate) {
      this.connections[i].mutateWeight();
      window['console'].log('#MUTATION');
    }
    if (Math.random() < toggle_enabled_rate) {
  ...
  if (Math.random() < add_node_rate && this.connections.length > 1) {
...
Network.prototype.draw = function(start_x, start_y, w, h) {
  ...
  for (let i = 0; i < this.layers; i++) {
    ...
    for (let j = 0; j < all_nodes[i].length; j++) {
