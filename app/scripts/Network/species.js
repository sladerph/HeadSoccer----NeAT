...
import {Unit} from '@/Network/genetics.js';

export function Species(player) {
  this.players = [];
  this.best_fitness = 0;
  this.best_ever;
  ...
  this.averageWeightDifference = function(a, b) {
    ...
    for (var i = 0; i < a.connections.length; i++) {
      for (var j = 0; j < b.connections.length; j++) {
        if (a.connections[i].innovation == b.connections[j].innovation) {
          matching++;
          total_difference += Math.abs(a.connections[i].innovation, b.connections[j].innovation);
          break;
        }
      }
    }
...
Species.prototype.areSameSpecies = function(genome) {
  ...
  window['console'].log(compatibility);
...
Species.prototype.selectPlayer = function() {
  ...
  var rand = Math.random() * fitness_sum;
  var actual_sum = 0;
...
Species.prototype.copulate = function(history) {
  ...
  if (Math.random() < 0.25) { // 25% of the time, the child is a clone of randomly one of the parents.
    ...
    if (parent1 && parent2) {
      ...
      window['console'].log('Error ! Parents are undefined');
    }
  }
