export function Node(id, layer, type) {
  this.id                  = id;
  this.layer               = layer;
  this.type                = type;
  ...
  this.isConnectedTo = function(node) {
    ...
    if (this.layer > node.layer) { // This node may an output of the other node.
      for (let i = 0; i < node.output_connections.length; i++) {
        if (node.output_connections[i].output == this) {
          return true;
        }
      }
